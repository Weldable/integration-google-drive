import { defineIntegration, createRestHandler, IntegrationValidationError, fakeId, fakeArray, fakeEmail, fakeIsoTimestamp, fakeUrl } from '@weldable/integration-core';
 ...
 */
function mimeClauseForType(type) {
...
export default defineIntegration({
    ...
    auth: {
        type: 'oauth2',
        test: async (_, ctx) => ctx.http.get('/about', { query: { fields: 'user' } }).then(r => r.data),
    },
    ...
    actions: [
        // ── Files ─────────────────────────────────────────────────
        {
            actionId: 'find',
            name: 'Find files',
            description: 'Find files in Google Drive by name, content, and type. Returns the most recently modified matches. Searches across all drives the user can access.',
            intents: [
                'find my google doc',
                'find a spreadsheet',
            ...
            outputFields: [
                {
                    name: 'files',
                    type: 'array',
                    description: 'Matching files, most recently modified first. Each has { id, name, type, url, modifiedAt, owner }.',
                },
                {
                    name: 'hasMore',
                    type: 'boolean',
            ...
            execute: async (args, ctx) => {
                ...
                if (type !== 'any' && !mimeClause && type !== '') {
                    throw new IntegrationValidationError(`unknown type "${type}". Allowed: any, document, spreadsheet, presentation, folder, pdf, image, video`, 'type');
                }
